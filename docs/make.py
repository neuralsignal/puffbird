...
To build the docs you must have all optional dependencies for puffbird
...
class DocBuilder:
    ...
    def __init__(
        ...
        if single_doc and single_doc.endswith(".rst"):
            ...
            self.single_doc_html = f"reference/api/puffbird.{single_doc}.html"

    def _process_single_doc(self, single_doc):
        """
        Make sure the provided value for --single is a path to an existing
        .rst/.ipynb file, or a puffbird object that can be imported.

        For example, categorial.rst or puffbird.FrameEngine. For the latter,
        return the corresponding file path
        (e.g. reference/api/puffbird.FrameEngine.rst).
        """
...
<html>"""
    ...
    def html(self):
        ...
        zip_fname = os.path.join(BUILD_PATH, "html", "puffbird.zip")
    ...
    def latex(self, force=False):
        ...
        if sys.platform == "win32":
            ...
            if force:
                for i in range(3):
                    self._run_os("pdflatex", "-interaction=nonstopmode", "puffbird.tex")
                raise SystemExit(
                    "You should check the file "
                    '"build/latex/puffbird.pdf" for problems.'
                )
            else:
                self._run_os("make")
            return ret_code
...
def main():
    ...
    argparser = argparse.ArgumentParser(
        description="puffbird documentation builder", epilog=f"Commands: {joined}",
    )
    ...
    globals()["puffbird"] = importlib.import_module("puffbird")
